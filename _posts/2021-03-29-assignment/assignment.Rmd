...
For easier and more effective data handling, this section outline the steps to ensure tidy data i.e. each variable is a column, each observation is a row and each type of observational unit is a table. In other words, tidy text becomes a table with one-token per row. 


***Data Types***
- if  host_since is already in date format, remain unchanged. Else run the code. 
...
***Cleaning***
- for comments, Remove anything but alphabets; Change all alphabets to lowercase
...
- anti_join remove stop words contained in the reviews/comments. 
...
**Frequency**
count() or n() can find the most common words that occured in the reviews.
...
```{r echo=TRUE}
subzone_data <- region_data %>% 
  group_by(neighbourhood_group_cleansed) %>% 
  summarise(mean_score=mean(score))

datatable(subzone_data)
```

** Read Shape File**

```{r echo=TRUE}
#mpsz <- st_read(dsn = "subzone-census2020/geospatial", layer = "MP14_SUBZONE_NO_SEA_PL")%>%
  #group_by(PLN_AREA_N) %>%
